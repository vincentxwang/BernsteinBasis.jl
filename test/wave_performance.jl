    ...
    function make_rhs_plot(K, samples)
        ...
        plot(groupedbar(ratio_times, bar_position = :dodge, group = group,color = [
            :cornflowerblue :coral :olivedrab
            ]),
            grid = false,
            title = "Speedup of Bernstein over nodal (wave system), \n min times over $(samples) samples",
        ...
        if (K < 8)
            plot!(x->1, c=:black, ls = :dash, lw = 2, linealpha = 0.5, label = "Baseline")
        end
    end 
